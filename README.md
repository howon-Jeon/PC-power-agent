...
Store the fixed AES key in the local `.env` file instead of source code. During build, `build.bat` embeds `PC_POWER_AES_KEY` into `installer.exe`; `.env` is not copied to `dist` and is ignored by Git.
